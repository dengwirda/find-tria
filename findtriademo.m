...
'   may become inefficient. \n\n']);
    ...
    pi = +6.*rand(100,size(pp,2));
    pi = pi - 3.*ones(size(pi));
   ...
   [tp,ti,tr] = findtria(pp,tt,pi,'pts',[]);
    ...
    patch('faces',tt(:,[3,4,1]),'vertices',pp,'facecolor',fc,...
        'edgecolor',ec,'facealpha',.3);
    plot3(pi(~isnan(tj),1),pi(~isnan(tj),2),pi(~isnan(tj),3),'b.','markersize',30);
    plot3(pi( isnan(tj),1),pi( isnan(tj),2),pi( isnan(tj),3),'r.','markersize',30);
    set(gca,'units','normalized','position',[0.01,0.05,.48,.90]);
    axis image off;
    aa = axis;
    view(3);
