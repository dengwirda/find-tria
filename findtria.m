...
%---------------------------------------------- basic checks    
    if (~isnumeric(pp) || ...
        ...
        ~isnumeric(pj) || ...
        ~ischar(ty)  )
        error('findtria:incorrectInputClass','Incorrect input class.') ;
    end
%---------------------------------------------- basic checks
    if (nargin < +3 || nargin > +6)
        error('findtria:incorrectNumInputs','Incorrect number of inputs. ');
    end
    if (ndims(pp) ~= +2 || size(pp,2) < +2 || size(pp,2) > size(tt,2))
        error('findtria:incorrectDimensions','Incorrect input dimensions.');
    end
    ...
    %----------------------------- query must be unsupported
        error('findtria:incorrectDimensions','Unsupported query type.') ;
    end
